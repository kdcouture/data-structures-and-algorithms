...
const count = (target, input) => {
  let count = 0;
  input.reduce( (accumeulator, value) => {
    return accumeulator.concat(value);
  }, []).forEach(ele => {
    if(ele === target)
      count++;
  })
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let tempSubArr = [];
  let retArr = [];
  retArr = input.reduce( (accumulator, value) => {
    tempArr = [];
    value.forEach(ele => {
      if(ele%5 === 0 && typeof(ele) !== 'string'){
        tempArr.push(ele);
      }
    });
...
let findMaleAndFemale = (data) => {
  let retStr = '';
  retStr = data.reduce( (accumeulator ,obj) => {
    if(obj.gender === 'male' || obj.gender === 'female'){
      accumeulator += (obj.name + ' and ');
    }
    return accumeulator;
  }, '');
...
let findShortest = (data) => {
  let min = 99999;
  return data.reduce( (accumeulator, obj) => {
    if(parseInt(obj.height) <= min) {
      accumeulator = obj.name;
      min = parseInt(obj.height);
    }
    return accumeulator;
  });
};
