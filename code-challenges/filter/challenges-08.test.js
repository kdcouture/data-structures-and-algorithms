...
let regxVowles = /[aeiou]/i;
const filterStringsWithVowels = (arr) => {
  return arr.filter(ele => (regxVowles.test(ele)));
};
...
const getStatName = (arr, minBaseStat) => {
  let retArr = [];
  let filterd = arr.filter(stat => {
    if(stat.baseStat > minBaseStat)
      return stat.stat.name;
  });
  filterd.forEach(ele => retArr.push(ele.stat.name));
  return retArr;
};
...
const evenOddNumericValues = (arr) => {
  let filterd = arr.filter(ele => typeof(ele) === 'number');
  return filterd.map(ele => {
    if(ele%2 === 0) {
      return 'even';
    }
    else {
