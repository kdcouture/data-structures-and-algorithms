...
const countNumberOfElements = (arr) => {
  let count = arr.reduce( (collector) => {
    return collector+=1;
  });
  return count;
};
...
const returnNames = (arr) => {
  return arr.reduce( (accumulator, val, idx) => {
...
const reversedString = (str) => {
  return str.split('').reduce( (newStr, currentChar) => {
    return currentChar + newStr;
  }, '');
};
...
const calculateAverage = (arr) => {
  let avgobj = arr.reduce( (accumulator, val) => {
    accumulator.count++;
    accumulator.sum += val;
    return accumulator;
  }, {count: 0, sum: 0});
